...
func drop(cfg *config.Config) {
	...
	if err != nil {
		log.Fatalf("[FATAL] root: Unknown user %s. %s", cfg.Username, err)
	}
	...
	if err := cmd.Start(); err != nil {
		log.Fatalf("[FATAL] root: Failed to start fabio as user %s", cfg.Username)
	}
	log.Printf("[INFO] root: Started fabio as user %s. Exiting", cfg.Username)
